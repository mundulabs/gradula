...
src/context.mjs      bounded source/document retrieval from a published graph — pure
tools/codegraph.mjs  local AST/document publisher for this repository (not the service)
...
14. **Retrieve context, not the whole graph or chronicle.** Use `gradula context "symbol or topic" --card KEY` or MCP `plan_context` for file discovery when a snapshot is available; inspect freshness, read the selected source, and fall back to local `rg` for missing/stale context (`tests/context.test.mjs`).
