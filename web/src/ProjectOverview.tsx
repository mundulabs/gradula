...
export default function ProjectOverview({project,cards,system,systemError,loading,open,board,retry}:{project:Project;cards:Card[];system:System|null;systemError:boolean;loading:boolean;open:(key:string)=>void;board:()=>void;retry:()=>void}){
  ...
  return <main className="project-workspace" aria-busy={loading}>
    <header className="project-hero"><div><span className="workspace-eyebrow">{project.key} / {t('workspace.project')}</span><h1>{project.name}</h1><p>{t('workspace.lede')}</p></div><div className="project-connection"><span className="connection-badge" data-state={systemError?'error':system?.observation?.refreshing||!system?'pending':'ready'}>{t(status)}</span><small>{observation?new Date(observation).toLocaleTimeString():t('workspace.firstObservation')}</small><button className="ghost" onClick={()=>{retry();setAttempt(n=>n+1)}}>{t('workspace.refresh')}</button></div></header>
    <section className="project-metrics" aria-label={t('workspace.work')}>
