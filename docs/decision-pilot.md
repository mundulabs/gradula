...
For this workstation, Gradula and Mundus may initially use the one user-supplied token in their respective private `.env` files. That is **shared provider billing**, not two independently issued keys. Separate project keys can replace those values later without any code or server credential change.
