...
test('knowledge projection bounds drawing, searches every node and preserves source relationships',()=>{
 ...
 const overview=projectGraph(graph,'','all',null);assert.ok(overview.shown.length<=36);assert.ok(overview.shown.some(n=>n.id==='119'));assert.ok(overview.shown.filter(n=>n.kind==='file').length<=9,'one column cannot overlap');
