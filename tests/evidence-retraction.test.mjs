...
for(const [name,build] of stores)test(`${name}: evidence can only be retracted by its submitter, atomically, with the original retained`,async t=>{
 ...
 await assert.rejects(g.retractEvidence(card.key,entry.id,'Correction','another actor'));
 ...
 const results=await Promise.allSettled([g.retractEvidence(card.key,entry.id,'Wrong branch inheritance','author'),g.retractEvidence(card.key,entry.id,'Duplicate attempt','author')]);
 ...
 assert.equal(history.find(e=>e.data?.retractedEvent===entry.id).actor,'author');
});
