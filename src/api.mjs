...
export function createApi(gradula, { adminToken = null, auth = null, staticFiles = null, origin = null, live = null, watcher = null, beat = counter() } = {}) {
  ...
  const routes = [
    ...
    ['PATCH', /^\/api\/v1\/project$/, async (req, _m, ctx) => {
      const body = await readJson(req);
      return { status: 200, body: await gradula.patchProject(ctx.project, { people: body.people, language: body.language, publish: body.publish, ladder: body.ladder }, ctx.actor) };
    }],
