import { CARD_STYLE, ladderOf } from './spec.mjs';
...
export function activitySummary(entries, project, { language = 'en', limit = 12 } = {}) {
  ...
  const lines = cards.slice(0, limit).map(card => `• ${card.key} ${ladderOf(card.state, CARD_STYLE)} ${card.state ?? ''}\n  ${String(card.title ?? '').replace(/\s+/g, ' ').slice(0, 120)}`);
