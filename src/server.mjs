...
const gradula = createGradula(store, { origin: process.env.PUBLIC_ORIGIN ?? null, live, houseKey: process.env.TELEGRAM_BOT_TOKEN || null, taskMeasurement: {projects:(process.env.GRADULA_MEASURED_PROJECTS||process.env.GRADULA_DECISION_PROJECTS||'').split(',').map(x=>x.trim()).filter(Boolean)} });
