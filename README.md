...
`gradula start KEY --files "src/audio,docs/audio.md"` reserves one card for the authenticated credential and a distinct session, then creates an isolated `codex/KEY` branch/worktree (from local `dev` when available). Update `dev` before starting. Enter the printed worktree and run `gradula work KEY` while working, or `gradula work KEY -- command …` for a bounded command. The explicit `--here "reason"` option keeps an already isolated checkout. Failed worktree setup releases the reservation and fails the command. Worktree creation does not copy uncommitted changes.
